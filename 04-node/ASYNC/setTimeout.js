...
// //실행 결과 1->2->3
// // => js에서는 setTimeout 함수를 사용했을 때 기다리지 않고 바로 다음 구문을 실행 함
...
//     // setTimeout 이용해 3초를 기다린 후에 코드 실행
//     setTimeout(function(){
...
//1. product,price 전역변수 확인 
//2. goMart(),pickDrink(),pay()함수 정의부 확인
//3. goMart() 함수 실행-> 콘솔 로그 찍음
//3. pickDrink()를 함수 실행
// 4-1 setTimeout함수의 의해 3초 대기
// 그런데 js 비동기 처리 -> 이코드가 끝날 때까지 기다리지 않고 다음 코드 실행
...
// js에 어떤 함수의 매개변수로 대입되는 함수

//목표: 콘솔로그 하나 찍고 3초 뒤에 고민 끝이랑 상품명, 가격이 올바르게 출력
...
product="제로";
price=3000;
Callback(product,price); // 매개변수로 받은 콜백함수를 실행
    },3000);
